...
```
Далее в папке куда был инсталирован проект необходимо запустить исполняемый файл, в качестве параметра необходимо передать JSON файл с конфигурацией, пример файла - `example.cpp`, результат проверки сохраняется в JSON файл, путь и название которого указано в конфигурационном файле
...
##### На выходе:
- Левая верхняя и правая нижняя координата зоны (большая зона может быть разделена на меньшие [текст на буквы и математические формулы]) с меткой, определяющей тип зоны.
